...
<body>
	<div class="container">
		<div class="row-fluid">
			<div class="span12">
				<!------------------- 지도 출력 ------------------->
				<div class="span9" id="mapSize" style="height: 600px">
					<div id="sortable_div" style="position: absolute; top: 5%; left: 5%; z-index: 5;">
...
<!-- 					<div id="dragbar"></div> -->
<!-- 					<div id="timeTableDiv" style="background-color: yellow;"> -->
...
<!-- 								li태그들어갈곳  -->
						...
						<li style="margin-bottom: 20px"><a href="#" onclick="addDetail()">저장하기</a></li>
						<li style="margin-bottom: 20px"><a href="#" onclick="loadDetail()">불러오기</a></li>
					</div>
				</div>
			</div>
		</div>
...
// 					+ '</div>';
			...
			var itemStr =  '<li class="media" onclick="listClickEvent('+location.lng+','+location.lat+')">'+
							'<a class="pull-left"><img class="media-object" src="./images/placeImg/'+location.tel+'.jpg"></a>'+
							'<div class="media-body">'+
							'<h5 class="media-heading">'+location.title+'</h5>'+
	...
	$.getJSON("json/" + cityTitle + "_Hot2.json", function(data) {
		...
		$(data.motels).each(function(i, motel) {
			//리스트출력시 html태그
// 			var itemStr2 = '<div class="info" onclick="listClickEvent('+motel.lng+','+motel.lat+')">'
// 					+ '   <h5>' + motel.title
// 					+ '</h5>';
// 			itemStr2 += '  <span>' + motel.lat
// 					+ '</span><br/>' + '  <span>'
// 					+ motel.lng + '</span><br/>'
// 					+ '</div>';

			var itemStr2 =  '<li class="media" onclick="listClickEvent('+motel.lng+','+motel.lat+')">'+
							'<a class="pull-left"><img class="media-object" src="./images/placeImg/'+motel.tel+'.jpg"></a>'+
							'<div class="media-body">'+
							'<h5 class="media-heading">'+motel.title+'</h5>'+
	...
	$.getJSON("json/" + cityTitle + "_Res2.json", function(data) {
		...
		$(data.restaurant).each(function(i, res) {
			//리스트출력시 html태그
// // 			var itemStr3 = '<div id="info">' + '<h5>'
// // 			var itemStr3 = '<div id="info" onclick="listClickEvent('+escape(res.title)+','+res.lng+','+res.lat+')">' + '<h5>'
// 			var itemStr3 = '<div id="info" onclick="listClickEvent('+res.lng+','+res.lat+')">' + '<h5>'
// 						+ res.title + '</h5>';
// 			itemStr3 += '  <span>' + res.lat
// 					+ '</span><br/>' + '  <span>' + res.lng
// 					+ '</span><br/>' + '</div>';
			var itemStr3 =  '<li class="media" onclick="listClickEvent('+res.lng+','+res.lat+')">'+
							'<a class="pull-left"><img class="media-object" src="./images/placeImg/'+res.tel+'.jpg"></a>'+
							'<div class="media-body">'+
							'<h5 class="media-heading">'+res.title+'</h5>'+
...
function fnAddEvent(markers){
	$(markers).each(function(index){
		...
		//등록 되어 있을때
		var iwContent3 = '<div style="padding:5px;"><a href="#theModal" id="register" style="text-align: center;" data-toggle="modal">'+marker.getTitle()+' 등록하기</a></div>'; // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
