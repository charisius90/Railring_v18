		...
		$.ajax({
			...
			success : function (data) {
				...
				for(var i=0; i<marker_arr.length; i++){
					...
					fnSetDefaultMarker(marker_arr[0]);
					// 지울 마커 sortable리스트에서 제거
 					$("#sort_li_" + marker_arr[0].getTitle()).remove();
					// 등록배열에서 마커 제거
					marker_arr.splice(0, 1);
				}
...
function myFunction(cityNo){
	...
	window.open(url, "세부일정", "width=950, height=627, toolbar=no, menubar=no, scrollbars=no, resizable=yes");
}
//***************** 2016-06-17 추가 부분 시작**********************
//polyline 관련 변수
...
function fnPlanMarkers(marker){
	...
	var liContents = "<li id='sort_li_"+ marker.getTitle() +"' class='ui-state-default' style='list-style:none;'>"
						...
						+ "<a style='display:inline-block;' href='javascript:fnDelMarker("+ param +")'>"
...
//     		var iwContent3 = '<div style="padding:5px;"><a href="#" onclick="myFunction()" style="text-align: center;" data-toggle="modal">'+position.title+' 세부일정</a></div>', // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
    		...
    		daum.maps.event.addListener(marker, 'click', function() {
    	        ...
    	        map.setCenter(new daum.maps.LatLng(position.lat, position.lng));
    	         ...
    	         //해당지역마커 클릭시
    	          $.getJSON("json/"+position.title+"_Loc.json",function(data){
				//alert(data.locations.length);
    	                ...
    	                $(data.locations).each(function(i,location){
    	                   ...
    	                   itemStr += '  <span>' + location.lat  + '</span><br/>' +
    	                            '  <span>' + location.lng  + '</span><br/>' +
    	                              '</div>';
	    	          ...
	    	          $.getJSON("json/"+position.title+"_Hot.json",function(data){
	    	                 //placesList의 childnode 삭제
	    	                 $('#placesList2').empty();
	    	                $(data.motels).each(function(i, motel){
	    	                   //리스트출력시 html태그
	    	                   var itemStr2 =   '<div class="info">' +
	    	                               '   <h5>' + motel.title + '</h5>';
	    	                               
	    	                   itemStr2 += '  <span>' + motel.lat  + '</span><br/>' +
	    	                            '  <span>' + motel.lng  + '</span><br/>' +
	    	                              '</div>';
	    	          ...
	    	          $.getJSON("json/"+position.title+"_Res.json",function(data){
	    	                 //placesList의 childnode 삭제
	    	                 $('#placesList3').empty();
	    	                $(data.restaurant).each(function(i,res){
	    	                   //리스트출력시 html태그
	    	                   var itemStr3 =   '<div class="info">' +
	    	                               '   <h5>' + res.title + '</h5>';
	    	                               
	    	                   itemStr3 += '  <span>' + res.lat  + '</span><br/>' +
	    	                            '  <span>' + res.lng  + '</span><br/>' +
	    	                              '</div>';
