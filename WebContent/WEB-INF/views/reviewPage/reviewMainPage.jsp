...
<%
	...
	Paging2<ReviewDto> paging = (Paging2<ReviewDto>) request.getAttribute("paging");
	ReviewReplyDao rdao = new ReviewReplyDao();
...
<!-- 			<div class="span12"> -->
				<div style="border: 1px solid gray; border-radius: 10px">
					<h4 style="margin-left: 40px;">후기 게시판</h4>
<!-- 					color: #2478FF -->
				</div>
				<hr />
				<div>
				<table align=center border=0 width=80%>
					...
					<thead>
						<tr>
							<th class="span1" style="text-align: center;">번호</th>
							...
							<th class="span1" style="text-align: center;">조회</th>
						</tr>
					</thead>
					<tbody>
						...
						<%} else {
								for(ReviewDto reviewDto : paging.getList()){
						%>
							<tr>
								<td style="text-align: center; vertical-align: middle;"><%=reviewDto.getRev_no()%></td>
<%-- 								<td><a href="review.action?cmd=READ&review_no=<%=reviewDto.getRev_no()%>"> --%>
...
<!-- 								</a></td> -->
								...
								<a href="review.action?cmd=READ&review_no=<%=reviewDto.getRev_no()%>">
		                           ...
		                           <%
		                              if(rdao.getSize(Integer.parseInt(reviewDto.getRev_no())) ==0 ){
		                                 
		                              }else{
		                           %>   
		                                 [<%=rdao.getSize(Integer.parseInt(reviewDto.getRev_no()))%>]</strong>
		                           <% 
		                              }
		                              if(diffTime < 1){
		                           %>
		                           ...
		                           %>
								<td style="text-align: center; vertical-align: middle;"><%=reviewDto.getMem_name()%></td>
				...
				<div align="right">
					<a class="btn" href="review.action?cmd=POST"><i class="icon-pencil"></i>글쓰기</a>
					<a class="btn" href="review.action?cmd=REVIEW&reload=true"><i class="icon-home"></i>처음으로</a>
					<!-- 연필아이콘 -->
				</div>
				<hr />
				<div align="center">Go to Page&nbsp;&nbsp;
					<%	
						if(paging.isPrev()){
					%>
						<a href="review.action?cmd=REVIEW&page=<%=paging.getPrevBlockPage()%><%if(paging.isSearchKeyword()){%>&keyword=<%=paging.getSearchKeyword()%>&searchOption=<%=paging.getSearchOption()%><%}%>">이전</a>&nbsp;:::&nbsp;&nbsp;
					<%	
						} 
					%>	
					<%
					...
					%>
								<a href="review.action?cmd=REVIEW&page=<%=i%><%if(paging.isSearchKeyword()){%>&keyword=<%=paging.getSearchKeyword()%>&searchOption=<%=paging.getSearchOption()%><%}%>"><%=i%></a>&nbsp;&nbsp;&nbsp;
					<%		
							}
						}
					%>
					<%	
						if(paging.isNext()){
					%>
						<a href="review.action?cmd=REVIEW&page=<%=paging.getNextBlockPage() %><%if(paging.isSearchKeyword()){%>&keyword=<%=paging.getSearchKeyword()%>&searchOption=<%=paging.getSearchOption()%><%}%>">다음</a>
					<%	
						} 
					%>
				</div>
				<br /> <br />
				<div align="center">
					<form name="searchForm" class="form-search" action="review.action">
